                    ...
                    @foreach($meals as $meal)
                        <div class="col-md-12 col-sm-12 col-xs-12 cart-list-main meal-product-img">
                            <div class="col-md-1 col-sm-2 col-xs-3"><img
                                        src="@if($meal->image1!=null){{ asset('uploads/'.$meal->image1) }}@else {{ asset('assets/frontend/img/background_img.jpg') }} @endif">
                            </div>
                            <div class="col-md-5 col-sm-4 col-xs-7">
                                <h4 class="card-product-title">@if($meal->status==1)<a
                                            href="{{ url($meal->slug) }}">{{ ucfirst($meal->title) }}</a>@else {{ ucfirst($meal->title) }} @endif</h4>
                            </div>
                            <div class="clear  product-price-list">
                                <div class="col-md-1 col-xs-2 col-sm-1 card-product-title"><span
                                            class="fixed-price">
                                     <?php if ($meal->type == 2) { ?>
                                        <b>&#8364; {{ number_format($meal->price + ($meal->price*($siteOptions->drink_vat_tax/100)),2) }}</b>

                                        <?php  }else{ ?>
                                        <b>&#8364; {{ number_format($meal->price + ($meal->price*($siteOptions->vat_tax/100)),2) }}</b>
                                        <?php }?>
                                    </span>
                                </div>
								...
								<span class="input-group-btn">
									<button type="button" class="btn btn-default btn-number cart-btn-padding" data-type="minus" data-field="quant[1]"><span
                                            ...
                                            <input type="text" name="quant[1]" class="form-control input-number" value="{{ $cart[$meal->id]['qty'] }}"
                                                   min="1" max="100" data-price="{{ $meal->price }}" data-id="{{ $meal->id }}">
                                            <span class="input-group-btn">
                                ...
                                <div class="col-md-1  col-sm-1 col-xs-2  card-product-title"><span class="fixed-price singleTotal{{ $meal->id }}">
                                     <?php if ($meal->type == 2) { ?>
                                        <b>&#8364; {{ number_format($cart[$meal->id]['qty']*($meal->price+ ($meal->price*
                                        ($siteOptions->drink_vat_tax/100))),2) }}</b>
                                        <?php  }else{ ?>
                                        <b>&#8364; {{ number_format($cart[$meal->id]['qty']*($meal->price+ ($meal->price*($siteOptions->vat_tax/100))),2) }}</b>
                                        <?php }?>
                                    </span>
                                </div>
                            </div>
                            <div class="col-md-1  col-sm-1 col-xs-2 card-product-title"><span data-id="{{ $meal->id }}" class="close remove-cart">&times;</span>
                            </div>
                        </div>
                    @endforeach
                @else
                    <div class="col-md-12 col-sm-12 col-xs-12 cart-list-main meal-product-img text-center">
                        ...
                        <a href="{{ url('/meals') }}" class="button-add">Continue Shopping</a>
                    </div>
                @endif
                @if(!empty($meals) && count($meals)>0)
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-6 cart_total">
                        <div class="col-md-12  cart-sidebar">
                            <b>Items <span
                                        id="itemTotal"> <b>&#8364; {{ number_format($cart['total'],2) }}</b></span></b>
                            <h5>Shipping Charge.
                                <span> <b>&#8364; {{ number_format($siteOptions->shipping_tax ,2) }}</b></span>
                            </h5>
                            <h5>MwSt. <span
                                        id="vatTax">
                                     <?php if ($meal->type == 2) { ?>
                                    <b>&#8364; {{ number_format(($cart['total']*($siteOptions->drink_vat_tax/100))  + ($siteOptions->shipping_tax *  ($siteOptions->drink_vat_tax/100)),2)}}</b>
                                    <?php  }else{ ?>
                                    <b>&#8364; {{ number_format(($cart['total']*($siteOptions->vat_tax/100))  + ($siteOptions->shipping_tax*($siteOptions->vat_tax/100)),2)}}</b>
                                    <?php }?>
                                </span></h5>
                            <h5>Total.<span
                                        id="cartTotal">&#8364;
                                    <?php if ($meal->type == 2) { ?>
                                    <b> {{ number_format(($cart['total'] + ($cart['total']*($siteOptions->drink_vat_tax/100))
                                    +$siteOptions->shipping_tax + ($siteOptions->shipping_tax*($siteOptions->drink_vat_tax/100))),2) }}</b>
                                    <?php  }else{ ?>
                                    <b> {{ number_format(($cart['total'] + ($cart['total']*($siteOptions->vat_tax/100))+$siteOptions->shipping_tax + ($siteOptions->shipping_tax*($siteOptions->vat_tax/100))),2) }}</b>
                                <?php }?>
                            </h5>
                        </div>
                        <div class="">
                            <form>
                                ...
                                <div class="select-field col-md-12">
                                    <a href="javascript:void(0);" data-link="{{ route('checkout') }}" id="do-checkout" class="button-add">Checkout</a>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        $('.input-number').on('change', function () {
            ...
            $.ajax({
                ...
                method: "post",
                data: {'_token': '{{ csrf_token()}}', mealId: mealId, qty: qty, price: price},
        ...
        function updateCart(response, id) {
            ...
            var shippingTax = parseFloat("{{ $siteOptions->shipping_tax }}");
            var vatTax = parseFloat("{{ $siteOptions->vat_tax }}");
            var drinkVatTax = parseFloat("{{ $siteOptions->drink_vat_tax }}");
            if (type == '2') {
        ...
        $('.remove-cart').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('updateCart') }}",
                method: "post",
                data: {'_token': '{{ csrf_token()}}', mealId: mealId},
                success: function (response) {
                    if (response.status == 200) {
